...
Since `dev` tasks don't produce outputs, `outputs` is empty. `dev` tasks are also special in that they're persistent (they don't exit by themselves), so we set `persistent` to `true`. This ensures that it doesn't block any other tasks from executing.
