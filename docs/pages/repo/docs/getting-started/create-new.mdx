...
Inside `dev`, we've specified `"persistent": true`. `dev` runs a persistent dev server and produces no outputs, so there is nothing to cache - so we mark it as `persistent`. Learn more about in our docs on [turning off caching](/repo/docs/core-concepts/caching#turn-off-caching).
