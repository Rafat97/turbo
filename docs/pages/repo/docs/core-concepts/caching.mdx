...
Sometimes, you _never_ want to cache a certain task. This could be a task you always want to run, like a database migration. Or, it could be a task that never completes, like a [`dev` task](/repo/docs/handbook/dev).

### Persistent tasks

For tasks that don't complete, like `dev` tasks or `--watch` tasks, we recommend using the `persistent` flag.
...
Flagging `persistent` as `true` marks this task as one that will never complete. This disables caching for that task, and also provides better introspection for Turbo to help you prevent infinite loops.

### Disabling caching

You can disable caching on specific tasks by setting the [`cache`](/repo/docs/reference/configuration#cache) flag to `false`:
...
This ensures that `db:migrate` will always be run, and its results will never be cached.

#### Via the CLI

To disable cache writes for the entire process, append `--no-cache` to any command:

```shell
# Run `dev` npm script in all workspaces in parallel,
# but don't cache the output
turbo run db:migrate --no-cache
```
